...
test('leerling vult alle rubricrijen op één scherm in en dient eenmaal in', () => {
  ...
  assert.match(stylesSource, /\.level-option\s*\{[^}]*display:\s*flex[^}]*flex-direction:\s*column[^}]*justify-content:\s*flex-start/s);
...
test('een testbeoordeling kan vanuit Resultaten worden verwijderd', () => {
  ...
  assert.match(appSource, /assessments\.filter\(\(item\) => item\.id !== deleted\.id\)/);
